# app.py — AI Budget Assistant (Fiscal-year aware, no uploads, auto category)
...
# =============================
# Professional CSS (palette-aware) + readable sidebar controls
# =============================
st.markdown(f"""
<style>
  ...
  /* Sidebar headings/labels text light */
  ...
  /* === Make sidebar selects and sliders readable (dark text on white) === */
...
def load_budget_pull(path: str) -> pd.DataFrame:
    """Load & normalize the official 'FY 2021 Budget Pull' file (fiscal-year aware: 1=Oct ... 12=Sep)."""
    raw = _read_any(path)
    ...
    for norm, orig in norm_cols.items():
        if norm in lookup:
            rename_map[orig] = canonical[lookup[norm]]
    ...
    if missing:
        raise ValueError(f"Budget Pull loader: missing {missing}")

    # Fill optionals if absent
    for k, v in {
    ...
    # --- Fiscal period -> calendar month mapping (1=Oct ... 12=Sep) ---
    FISCAL_TO_CAL_MONTH = {1:10, 2:11, 3:12, 4:1, 5:2, 6:3, 7:4, 8:5, 9:6, 10:7, 11:8, 12:9}

    # Accept digits, yyyy-mm, or month names in Accounting_Period
    ...
    df["Accounting_Period"] = pd.to_numeric(df["Accounting_Period"].apply(_coerce_period), errors="coerce")
    ...
    # Map to calendar month & year (Oct–Dec belong to prior calendar year)
    df["Cal_Month"] = df["Accounting_Period"].map(FISCAL_TO_CAL_MONTH).astype("Int64")
    ...
    # Build Month (calendar month start)
    ...
    # Variance & NA-safe variance %
    df["Variance"] = df["Actual_Spent"] - df["Budget_Allocated"]
    den = df["Budget_Allocated"].replace({0: np.nan})
    df["Variance_Percent"] = ((df["Variance"] / den) * 100)
    df["Variance_Percent"] = pd.to_numeric(df["Variance_Percent"], errors="coerce").round(2)
...
# =============================
ACTUALS_PATH = "FY 2021 Budget Pull.xlsx"  # supply this at repo root
...
with st.sidebar:
    ...
    with c1:
        ...
        st.metric("🏢 Depts", f"{df['Department'].nunique():,}" if "Department" in df.columns else "—")
    with c2:
        st.metric("📁 Account Types", f"{df['Account_Type'].nunique():,}" if "Account_Type" in df.columns else "—")
    ...
    # --- Auto-pick a category dimension (no UI control) ---
    PRIORITY = [
        ("Account Type", "Account_Type"),
        ("Account Description", "Account_Desc"),
        ("Program Description", "Program_Desc"),
    ...
    CAT_FIELD, category_label = None, "Category"
    for label, col in PRIORITY:
        if col in df.columns and df[col].notna().any():
            CAT_FIELD, category_label = col, label
            break
    if CAT_FIELD is None:
        CAT_FIELD, category_label = "Department", "Department"

    # Read-only indicator of the active category
    st.markdown(f"**📁 Category:** {category_label}")

    dept_opts = sorted(df["Department"].dropna().unique()) if "Department" in df.columns else []
    cat_opts = sorted(df[CAT_FIELD].dropna().unique())

    dept_sel = st.multiselect("🏢 Department(s)", dept_opts, default=dept_opts) if dept_opts else []
    cat_sel = st.multiselect(f"📁 {category_label}(s)", cat_opts, default=cat_opts)

    # --- Robust Month slider (works even if only one month exists) ---
    min_ts = pd.to_datetime(df["Month"].min())
    max_ts = pd.to_datetime(df["Month"].max())
    if pd.isna(min_ts) or pd.isna(max_ts):
        st.error("No valid dates found in Month column.")
        st.stop()

    min_m = min_ts.to_pydatetime()
    max_m = max_ts.to_pydatetime()

    if min_m >= max_m:
        # single-month dataset → create a tiny 1-month window so the range slider can render
        faux_max = (pd.Timestamp(min_m) + pd.offsets.MonthBegin(1)).to_pydatetime()
        st.caption("ℹ️ Only one month available; showing a 1-month slider window.")
        date_range = st.slider(
            "📅 Month Range",
            value=(min_m, faux_max),
            min_value=min_m,
            max_value=faux_max,
            format="YYYY-MM",
        )
    ...
    # Effective columns preview for slider ranges
    df_preview = df.copy()
    ...
    with st.expander("Additional Filters (Fund / Program / Account / Ledger)"):
        ...
        sel_acct = st.multiselect("🧾 Account Type",
                                  sorted(df["Account_Type"].dropna().unique())) if "Account_Type" in df.columns else []
...
mask = (
    (df_work[CAT_FIELD].isin(cat_sel) if cat_sel else True) &
...
# Additional filters
if "Fund_Desc" in df_work.columns and 'sel_fund' in locals() and sel_fund:
    mask &= df_work["Fund_Desc"].isin(sel_fund)
if "Program_Desc" in df_work.columns and 'sel_prog' in locals() and sel_prog:
    mask &= df_work["Program_Desc"].isin(sel_prog)
if "Account_Type" in df_work.columns and 'sel_acct' in locals() and sel_acct:
    mask &= df_work["Account_Type"].isin(sel_acct)
if "Ledger_Group" in df_work.columns and 'sel_ledger' in locals() and sel_ledger:
...
tab1, tab2, tab3, tab4 = st.tabs(["📊 Monthly Trends", "🏢 By Department", f"🗂️ By {category_label}", "🔎 By Any Column"])
...
with tab3:
    cat_key = CAT_FIELD
    cat_tot = df_f.groupby(cat_key, as_index=False).agg(
        Allocated=("Budget_Allocated","sum"),
        Spent=("Actual_Effective","sum")
    )
    cat_tot["Variance"] = cat_tot["Spent"] - cat_tot["Allocated"]
    order_cat = cat_tot.sort_values("Variance", ascending=False)[cat_key].astype(str).tolist()
    cat_long = cat_tot.melt(id_vars=cat_key, value_vars=["Allocated","Spent"],
                            var_name="Type", value_name="Amount")
    chart_cat = (
        alt.Chart(cat_long)
        .mark_bar(cornerRadius=3)
        .encode(
            x=alt.X(f"{cat_key}:N", sort=order_cat, axis=alt.Axis(labelAngle=-45), title=category_label),
            y=alt.Y("Amount:Q", axis=alt.Axis(format="$,.0f"), title="Amount ($)"),
            color=alt.Color("Type:N", title="", scale=alt.Scale(domain=["Allocated","Spent"],
                                                                range=[pal["alloc"], pal["spent"]])),
            xOffset="Type:N",
            tooltip=[alt.Tooltip(f"{cat_key}:N", title=category_label), alt.Tooltip("Type:N"),
                     alt.Tooltip("Amount:Q", title="Amount", format=",.0f")]
        ).properties(height=420)
    )
...
def build_compact_summary(actuals_df: pd.DataFrame, cat_field: str, cat_label: str):
    ...
    a_cat = (actuals_df.groupby(cat_field, as_index=False)[["Actual_Effective","Budget_Allocated"]]
             .sum().sort_values("Actual_Effective", ascending=False).head(10)) if cat_field in actuals_df.columns else pd.DataFrame()
    prompt = f"""
...
with cols[0]:
    ...
    if st.button("🔍 Generate Smart Summary", use_container_width=True):
        if not client:
            ...
            compact = build_compact_summary(df_f, CAT_FIELD, category_label)
...
with cols[1]:
    ...
    q = st.text_input(
        ...
        placeholder=f"e.g., 'Actual spent in 2021-06 for HR', 'Budget for {category_label} X 2021-11', '{category_label} Supplies variance 2021'"
    )
    if q:
        ...
        for dim in ["Department","Fund_Desc","Program_Desc","Account_Type","Account_Desc","Ledger_Group", CAT_FIELD]:
        ...
        if CAT_FIELD in df_q.columns and CAT_FIELD not in send_cols: send_cols.append(CAT_FIELD)
...
<div class="caption-style">
  <strong>Pro Tips:</strong> Place <code>FY 2021 Budget Pull.xlsx</code> (or .csv/.xls) at repo root.
  Toggle "Include Encumbrances" to treat commitments as spend. All charts/insights respect your filters.
  Fiscal period 1=October … 12=September; the timeline uses true calendar order for clarity.
</div>
""", unsafe_allow_html=True)
