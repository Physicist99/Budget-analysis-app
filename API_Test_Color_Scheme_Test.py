...
THEMES = {
    "Executive Light": {
        "bg": "#F7FAFC",        # page background (very light)
        "sidebar": "#0B1E3E",   # deep navy sidebar
        "panel": "#FFFFFF",
        "card": "#FFFFFF",
        "text": "#0F172A",      # main text (slate-900)
        "muted": "#475569",     # muted text (slate-600)
        "grid": "#E2E8F0",      # gridlines (slate-200)
        # Brand gradient
        "brand1": "#0B1E3E",    # midnight navy
        "brand2": "#2F6BFF",    # royal blue
        # Chart/semantic
        "alloc": "#2151B8",     # allocated (corporate blue)
        "spent": "#53A2FF",     # spent (cerulean)
        "forecast": "#16A34A",  # forecast (emerald)
        "warn": "#D97706",
        "ok": "#10B981"
    },
...
# =============================
# Professional CSS (palette-aware)
# =============================
st.markdown(f"""
<style>
  ...
  /* Inputs / labels in main area */
  .stSelectbox div, .stMultiSelect div, .stSlider div, label, .stTextInput, .stNumberInput, .stDateInput {{
    color: {pal['text']};
  }}
...
def load_actuals(path: str) -> pd.DataFrame:
    ...
    # Variance optional -> compute if missing
    req_core = ["Month","Department","Category","Budget_Allocated","Actual_Spent"]
    miss_core = [c for c in req_core if c not in df.columns]
    if miss_core:
        raise ValueError(f"Actuals missing columns: {miss_core}")
...
try:
    ...
    st.error(f"❌ Could not load actuals: {e}")
    st.stop()

try:
    forecast = load_forecast(FORECAST_PATH)
except Exception as e:
    st.warning(f"⚠️ Forecast file issue: {e}")
    forecast = None
