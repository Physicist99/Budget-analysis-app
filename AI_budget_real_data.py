...
def load_budget_pull(path: str):
    ...
    Returns standardized columns (subset):
...
if df is None:
    ...
    <div style="padding:1.25rem;border:1px dashed #94a3b8;border-radius:12px;background:#ffffff0d">
      ...
      or set an environment variable <code>BUDGET_DATA_PATH=/absolute/path/to/file.xlsx</code>.
      </p>
      <ul style="margin:.5rem 0 0 1rem;">
        <li>Supported: CSV / XLSX / XLS</li>
        <li>Required headers (synonyms accepted): <em>BUDGET YEAR, ACCOUNTING PERIOD, BUDGET AMOUNT, EXPENSE AMOUNT</em></li>
      </ul>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
with tab3:
    cat_tot = df_f.groupby("Account_Type" if "Account_Type" in df_f.columns else CAT_FIELD, as_index=False).agg(
        Allocated=("Budget_Allocated","sum"),
        Spent=("Actual_Effective","sum")
    )
    cat_tot["Variance"] = cat_tot["Spent"] - cat_tot["Allocated"]
    cat_key = "Account_Type" if "Account_Type" in df_f.columns else CAT_FIELD
