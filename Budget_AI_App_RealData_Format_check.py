# app.py — AI Budget Assistant (Auto-discover file • Executive themes • Scalable AI)
import os
import re
...
alloc_col, spent_col = pal["alloc"], pal["spent"]  # to avoid any quoting typos
...
def discover_budget_file() -> str | None:
    """Find a file whose name looks like 'FY 2021 Budget Pull' anywhere in repo."""
    roots = [Path("."), Path("./data"), Path("./datasets"), Path("./input")]
    exts = (".xlsx", ".csv", ".xls")
    for root in roots:
        if not root.exists():
            continue
...
def _read_any(path: str) -> pd.DataFrame:
    if path.lower().endswith((".xlsx",".xls")):
        # requires openpyxl for .xlsx; xlrd (optional) for old .xls
...
def load_budget_pull_with_raw(path: str):
    ...
    # RAW unique lists (full options for selectors)
    raw_dept_col = _find_raw_col(raw, "DEPARTMENT ID DESCRIPTION")
    raw_accd_col = _find_raw_col(raw, "ACCOUNT DESCRIPTION")
    raw_depts = sorted(raw[raw_dept_col].dropna().astype(str).str.strip().unique().tolist()) if raw_dept_col else []
    raw_accds = sorted(raw[raw_accd_col].dropna().astype(str).str.strip().unique().tolist()) if raw_accd_col else []
    raw_counts = {"raw_dept_n": len(raw_depts), "raw_accdesc_n": len(raw_accds)}

    # Clean → used (calendarize fiscal -> calendar)
    ...
    df["Month"] = pd.to_datetime(df["Cal_Year"].astype(str) + "-" + df["Cal_Month"].astype(str).str.zfill(2) + "-01", errors="coerce")
    df = df.dropna(subset=["Month"]).copy()

    # Helpers + effective measures
...
# === Resolve data path (env override → discover in repo) ===
DATA_PATH = os.getenv("BUDGET_DATA_PATH") or discover_budget_file()
if not DATA_PATH:
    # show inventory to help user
    inventory = []
    for ext in ("*.xlsx", "*.csv", "*.xls"):
        inventory += [str(p) for p in Path(".").rglob(ext)]
    st.error("❌ Could not find your budget file named like 'FY 2021 Budget Pull'.")
    if inventory:
        st.markdown("**Here are the data files I can see:**")
        st.code("\n".join(inventory[:200]))
    st.stop()

try:
    df, raw_df, raw_counts, used_counts, raw_depts, raw_accds = load_budget_pull_with_raw(DATA_PATH)
    st.sidebar.info(f"Loaded: {os.path.basename(DATA_PATH)}")
except Exception as e:
    st.error(f"❌ Failed to load data at {DATA_PATH}: {e}")
    st.stop()
...
with st.sidebar:
    ...
    # --- Primary Filters (RAW options for choices; USED defaults so first render shows data) ---
    st.markdown('<div class="filter-header">🔍 Primary Filters</div>', unsafe_allow_html=True)
    raw_dept_opts = sorted(raw_depts)
    raw_accd_opts = sorted(raw_accds)
    used_dept_defaults = sorted(df["Department"].dropna().astype(str).unique().tolist()) if "Department" in df.columns else []
    used_accd_defaults = sorted(df["Account_Desc"].dropna().astype(str).unique().tolist()) if "Account_Desc" in df.columns else []
    used_type_defaults = sorted(df["Account_Type"].dropna().astype(str).unique().tolist()) if "Account_Type" in df.columns else []

    dept_sel = st.multiselect("Department", raw_dept_opts, default=used_dept_defaults or raw_dept_opts)
    accd_sel = st.multiselect("Account Description", raw_accd_opts, default=used_accd_defaults or raw_accd_opts)

    acct_type_opts = sorted(df["Account_Type"].dropna().astype(str).unique()) if "Account_Type" in df.columns else []
    acct_type_sel = st.multiselect("Account Type", acct_type_opts, default=used_type_defaults)
    ...
    # Recompute effective series for slider ranges (if commitments included)
    df_tmp = df.copy()
    if include_commitments:
        ...
        den_tmp = df_tmp["Budget_Allocated"].replace({0: np.nan})
        df_tmp["Variance_Percent_Effective"] = (df_tmp["Variance_Effective"] / den_tmp) * 100
    ...
    with st.expander("🔎 Debug (data health)"):
        st.write({
            "raw_rows": len(raw_df),
            "used_rows": len(df),
            "used_departments": int(df["Department"].dropna().nunique()) if "Department" in df.columns else 0,
            "used_acc_desc": int(df["Account_Desc"].dropna().nunique()) if "Account_Desc" in df.columns else 0,
            "date_span": (str(pd.to_datetime(df["Month"].min()))[:10], str(pd.to_datetime(df["Month"].max()))[:10]),
        })
...
<div style="margin-top:.5rem;opacity:.85">
  <em>Why raw ≠ used?</em> We compute calendar months from fiscal period (1=Oct … 12=Sep) and budget year.
  Rows with missing/invalid period or year can’t be placed on a timeline, so they’re excluded from analytics.
  Primary filter options come from the raw file (full lists), while charts and KPIs use the cleaned, dated rows.
</div>
""",
    unsafe_allow_html=True
)
