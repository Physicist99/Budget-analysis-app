# app.py — AI Budget Assistant (Fiscal-year aware)
...
def load_budget_pull(path: str) -> pd.DataFrame:
    ...
    # Calendar helpers (if needed elsewhere)
...
with st.sidebar:
    ...
    with c1:
        ...
        st.metric("🏢 Departments", f"{df['Department'].nunique():,}" if "Department" in df.columns else "—")
    with c2:
    ...
    # Category dimension options (existence-safe)
    ...
    cat_field_display_map = {k: v for k, v in cat_field_display_map.items() if v in df.columns and df[v].notna().any()}
    if not cat_field_display_map:
        cat_field_display_map = {"Department": "Department"}
    ...
    cat_sel = st.multiselect(f"📁 {category_choice}(s)", cat_opts, default=cat_opts)
...
with tab3:
    cat_key = CAT_FIELD if CAT_FIELD in df_f.columns else ("Department" if "Department" in df_f.columns else None)
    if cat_key:
        ...
        chart_cat = (
            ...
            .mark_bar(cornerRadius=3)
            .encode(
                x=alt.X(f"{cat_key}:N", sort=order_cat, axis=alt.Axis(labelAngle=-45), title="Category"),
                y=alt.Y("Amount:Q", axis=alt.Axis(format="$,.0f"), title="Amount ($)"),
                color=alt.Color("Type:N", title="", scale=alt.Scale(domain=["Allocated","Spent"],
                                                                    range=[pal["alloc"], pal["spent"]])),
                xOffset="Type:N",
                tooltip=[alt.Tooltip(f"{cat_key}:N", title="Category"), alt.Tooltip("Type:N"),
                         alt.Tooltip("Amount:Q", title="Amount", format=",.0f")]
            ).properties(height=420)
        )
        st.altair_chart(chart_cat, use_container_width=True)
    else:
        st.info("No category field available to chart.")
...
ACTUALS — top by {cat_field if cat_field in actuals_df.columns else 'category'}:
{tbl(a_cat)}
"""
    return prompt
...
with cols[0]:
    ...
    if st.button("🔍 Generate Smart Summary", use_container_width=True):
        if not client:
            ...
            compact = build_compact_summary(df_f, CAT_FIELD if CAT_FIELD in df_f.columns else ("Department" if "Department" in df_f.columns else ""))
