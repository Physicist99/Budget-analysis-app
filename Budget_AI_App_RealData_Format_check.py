# app.py — AI Budget Assistant (Executive themes • raw→used counts • scalable AI)
...
<style>
  ...
  /* Make sidebar controls readable: white background + dark text */
...
def load_budget_pull_with_raw(path: str):
    ...
    # Helpers + effective measures (commitments toggle later)
...
# =============================
# Load data (fixed filenames: xlsx → csv → xls)
# =============================
...
try:
    ...
    for alt in [base + ".csv", base + ".xls"]:
        ...
        if os.path.exists(alt):
            ...
            break
    if df is None:
        st.error(f"❌ Could not load data at {DATA_PATH} (also tried {', '.join(tried)}).")
        st.stop()
...
with st.sidebar:
    ...
    # --- Primary Filters (RAW options, USED defaults) ---
    st.markdown('<div class="filter-header">🔍 Primary Filters</div>', unsafe_allow_html=True)
    raw_dept_opts = raw_depts
    raw_accd_opts = raw_accds
...
# =============================
# KPIs (show helpful message if empty, but keep app visible)
# =============================
