...
def load_budget_pull(path: str) -> pd.DataFrame:
    """Load & normalize the official 'FY 2021 Budget Pull' file."""
    raw = _read_any(path)
...
def smart_load_fixed() -> pd.DataFrame:
    """
    Fixed logic (no dummy fallback, no uploads):
      - Try official names at repo root, then ./data/
    """
    candidates = [
        "FY 2021 Budget Pull.xlsx",
        "FY 2021 Budget Pull.csv",
        "FY 2021 Budget Pull.xls",
        os.path.join("data", "FY 2021 Budget Pull.xlsx"),
        os.path.join("data", "FY 2021 Budget Pull.csv"),
        os.path.join("data", "FY 2021 Budget Pull.xls"),
    ]
    last_err = None
    for path in candidates:
        if os.path.exists(path):
            try:
                return load_budget_pull(path)
            except Exception as e:
                last_err = e
                continue
    if last_err: raise last_err
    raise FileNotFoundError("No ‘FY 2021 Budget Pull’ file found in repo root or ./data/.")
...
with st.sidebar:
    ...
    cat_field_display_map = {k: v for k, v in cat_field_display_map.items() if v in df.columns}
    ...
    with st.expander("Additional Filters (Fund / Program / Account / Ledger)"):
        fund_sel = df["Fund_Desc"].dropna().unique().tolist() if "Fund_Desc" in df.columns else []
        prog_sel = df["Program_Desc"].dropna().unique().tolist() if "Program_Desc" in df.columns else []
        acc_type_sel = df["Account_Type"].dropna().unique().tolist() if "Account_Type" in df.columns else []
        ledger_sel = df["Ledger_Group"].dropna().unique().tolist() if "Ledger_Group" in df.columns else []

        sel_fund = st.multiselect("🏦 Fund Description", sorted(fund_sel), default=sorted(fund_sel)) if fund_sel else []
        sel_prog = st.multiselect("📘 Program Description", sorted(prog_sel), default=sorted(prog_sel)) if prog_sel else []
        sel_acct = st.multiselect("🧾 Account Type", sorted(acc_type_sel), default=sorted(acc_type_sel)) if acc_type_sel else []
        sel_ledger = st.multiselect("📚 Ledger Group", sorted(ledger_sel), default=sorted(ledger_sel)) if ledger_sel else []
...
# Additional filters (only if present/selected)
...
<div class="main-header">
  <h1>AI Budget Forecast & Analysis</h1>
  <p style="font-size:1.05rem;margin:.4rem 0;">Professional Visual Analytics</p>
...
cols_show = [
    "Month_Display","Year","Accounting_Period","Department",
...
st.dataframe(
    display_df[cols_show].rename(columns={
        "Month_Display":"Month", "Accounting_Period":"Period",
...
with tab3:
    cat_key = CAT_FIELD if CAT_FIELD in df_f.columns else "Department"
...
with cols[0]:
    ...
    if st.button("🔍 Generate Smart Summary", use_container_width=True):
        if not client:
            ...
            compact = build_compact_summary(df_f, CAT_FIELD if CAT_FIELD in df_f.columns else "Department")
...
<div class="caption-style">
  <strong>Pro Tips:</strong> Place <code>FY 2021 Budget Pull.xlsx</code> (or .csv/.xls) at repo root or in <code>./data/</code>.
