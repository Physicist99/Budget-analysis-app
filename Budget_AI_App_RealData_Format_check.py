# app.py — AI Budget Assistant (scalable, fiscal-aware, Exec Dark default)
...
# ========= Sidebar: Theme (default = Executive Dark) =========
with st.sidebar:
    st.markdown("**🎨 Theme**")
...
<style>
  ...
  /* Sidebar inputs readable (dark text on white) */
...
with st.sidebar:
    ...
    # Primary: Department, Account Description, Account Type
    ...
    # Additional Filters (no Account Type/Desc here by request)
...
<div class="main-header">
  ...
  <p style="font-size:.9rem;opacity:.9;">Actuals vs Budget with encumbrance option & primary filters for Dept/Account</p>
</div>
""", unsafe_allow_html=True)
...
with cols[1]:
    ...
    if q:
        # Narrow by month if the user specified one
        y, m = parse_month_year(q)
        df_q = df_f.copy()
        if y and m:
            df_q = df_q[df_q["Month"].dt.strftime("%Y-%m") == f"{y}-{m:02d}"]

        # Detect primary grouping dimension from question
        ql = q.lower()
        prefer_dim = None
        ...
        # If the user names a specific value, filter to it
        for dim in ["Department","Account_Type","Account_Desc","Fund_Desc","Program_Desc","Ledger_Group"]:
            if dim in df_q.columns:
                ...
                if match:
                    ...
                    if prefer_dim == dim:
                        prefer_dim = None  # already narrowed to a single group

        # Helper: pretty string for prompt
        ...
        # Build primary context: AGGREGATE FIRST
        coverage_note = ""
        if prefer_dim and prefer_dim in df_q.columns and not df_q.empty:
            ...
            # Cap to slider size with coverage stats
            ...
            # Fallback: sample raw rows, sorted by |variance|, capped by slider
        ...
        # Backstop summaries (small, stable)
        compact = build_compact_summary(df_f)
